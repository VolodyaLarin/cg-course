...
private:
    std::mutex _m;
public:
    GLuint RBO_Normal, RBO_Small, FBO_CZ, FBO_VL, FBO_TM, FBO_NM, FBO_GA, FBO_PixelBlur,
            TEX_C, TEX_Z, TEX_VL, TEX_TM, TEX_NM, TEX_GA, TEX_PixelBlur;
    ...
    void allocateTextures() {
        ...
        /* Pixel blur */
        glGenTextures(1, &TEX_PixelBlur);
        glBindTexture(GL_TEXTURE_2D, TEX_PixelBlur);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    void allocateFrameBuffers() {
        ...
        /* Pixel blur */
        glGenFramebuffers(1, &FBO_PixelBlur);
        glBindFramebuffer(GL_FRAMEBUFFER, FBO_PixelBlur);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, TEX_PixelBlur, 0);
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, RBO_Normal);
        glDrawBuffers(1, DrawBuffers);
    }
    ...
    void clear() {
        ...
        glDeleteFramebuffers(1, &FBO_PixelBlur);
        ...
        glDeleteTextures(1, &TEX_PixelBlur);
    }
